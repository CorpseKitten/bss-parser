...
{
    public class LifeExpConverter
    {
        public static void CreateXml(List<LifeExp> Entries)
        {
            var document = new XDocument(
                new XElement("datas",
                    new XAttribute("clearTable", true),
                    new XAttribute("method", "insert"),
                    new XAttribute("table", "LifeEXP_Table"),
            ...
            using (var xw = XmlWriter.Create(Path.Combine("output.xml", "LifeEXP_Table.xml"), xws))
                document.Save(xw);
        }
        public static void CreateSql(List<ActionExp> Entries)
        {
            var sb = new List<string> { QueryBuilder.Prepare("LifeEXP_Table") };
            foreach (var entry in Entries)
            {
                var qb = new QueryBuilder("LifeEXP_Table");
            ...
            File.WriteAllLines(Path.Combine("output.sql", "LifeEXP_Table.sql"), sb);
        }
    }
}
